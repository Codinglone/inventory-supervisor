...
$logged_user = $_SESSION['supermarket'];
...
if (isset($_POST['sellProduct'])) {
    ...
    // Validate inputs
    $supermarket_product_id = trim($_POST['productNameSupermarket']);
    $supermarket_quantity = filter_var($_POST['quantitySupermarket'], FILTER_VALIDATE_INT);

    if (empty($supermarket_product_id) || !$supermarket_quantity) {
        $error = 'Invalid product data';
    } else {
        $stmt_product = $link->prepare("SELECT * FROM supermarket_products WHERE id = ?");
        $stmt_product->bind_param("i", $supermarket_product_id);
        $stmt_product->execute();
        $result_product = $stmt_product->get_result();
        $row = $result_product->fetch_assoc();
        // Insert with prepared statement 
        $stmt = $link->prepare("INSERT INTO supermarket_products_transactions(product_name, quantity, price_per_unit,dates) VALUES(?, ?, ?, ?)");
        $date = date("Y-m-d");
        $stmt->bind_param("sids", $row['product_name'], $supermarket_quantity, $row['unit_price'], $date);
        if ($row['quantity'] < $supermarket_quantity) {
            echo "<script>alert('You have only " . $row['quantity'] . " " . $row['product_name'] . " in stock')</script>";
        } else {
            $remaining_product_quantity = $row['quantity'] - $supermarket_quantity;
            $stmt_update = $link->prepare("UPDATE supermarket_products SET quantity=? WHERE id=?");
            $stmt_update->bind_param("ii", $remaining_product_quantity, $supermarket_product_id);
            if (!$stmt_update->execute() || !$stmt->execute()) {
                $error = $stmt->error;
            } else {
                ...
                <div class="tab-content">
                    <div class="tab-pane fade show active" id="cyberProducts">
                        ...
                        <div class="card p-3" style="width: 800px;">
                            <form method="POST">
                                <div class="mb-3">
                                    <label for="productNameSupermarket" class="form-label">Product Name</label>
                                    <select class="form-select" name="productNameSupermarket" id="productNameSupermarket
                                ...
                                <div class="mb-3">
                                    <label for="quantitySupermarket" class="form-label">Quantity</label>
                                    <input type="number" class="form-control" id="quantitySupermarket"
                                        name="quantitySupermarket" required>
                                </div>
