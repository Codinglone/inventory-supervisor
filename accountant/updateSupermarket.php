      ...
      <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4 d-flex justify-center">
        <div class="tab-content">
          <div class="tab-pane fade active show" id="cyberView">
            ...
            <h3 class="mb-4">Update Supermarket Product</h3>
            <?php 
            if(isset($_GET['id'])){
                $id = $_GET['id'];
                $sql = "SELECT * FROM supermarket_products WHERE id = '$id'";
                $result = $link->query($sql);
                $row = mysqli_fetch_array($result);

                if(isset($_POST['updateSupermarketProduct'])){
                    $product_update = $link->query("UPDATE supermarket_products SET product_name = '$_POST[productNameSupermarket]', unit_price = '$_POST[unitPriceSupermarket]', quantity = '$_POST[quantitySupermarket]' WHERE id = '$id'");
                    if($product_update){
            ...
            <div class="card p-3" style="width: 800px;">
              <form method="POST">
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" value="<?php echo $row['product_name'] ?>" id="productNameSupermarket" name="productNameSupermarket" required>
                </div>
                <div class="mb-3">
                  <label for="unitPriceCyber" class="form-label">Unit Price in RWF</label>
                  <input type="number" class="form-control"  value="<?php echo $row['unit_price'] ?>" id="unitPriceSupermarket" name="unitPriceSupermarket" required>
                </div>
                <div class="mb-3">
                  <label for="quantityCyber" class="form-label">Quantity</label>
                  <input type="number" class="form-control"value="<?php echo $row['quantity'] ?>" id="quantitySupermarket" name="quantitySupermarket" required>
                </div>
                <button type="submit" name="updateSupermarketProduct" class="btn btn-primary">Update Product</button>
              </form>
            </div>
            <?php } ?>
          </div>
